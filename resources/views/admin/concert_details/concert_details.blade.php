<x-app-layout>
    ...
    <div class="px-4 w-3/4">
        <div class="px-5 py-2 mx-auto my-3 hover:text-white bg-gray-300 hover:bg-white shadow-lg rounded-lg w-fit">
            <a href=" {{ route('admin.addConcertDetails') }}"  class="text-black">Add Concert Details</a>
        </div>
                            ...
                            <tr class="bg-white border-b hover:bg-gray-100 ">
                                ...
                                <td class="text-center px-6 py-4">
                                    @foreach($concert_id as $concert)
                                        @if($concert->id == $concert_details->concert_id)
                                            {{ $concert->name }}
                                        @endif
                                    @endforeach
                                </td>
                                <td class="text-center px-6 py-4">
                                    @foreach($venue_id as $venue)
                                        @if($venue->id == $concert_details->venue_id)
                                            {{ $venue->name }}
                                        @endif
                                    @endforeach
                                </td>
                                <td class="flex justify-evenly items-center px-6 py-4 text-right">
                                    <a href="{{ route('admin.editConcertDetails', ['id' => $concert_details->id])  }}" class="font-medium  hover:underline">Edit</a>
                                    <a href="{{ route('admin.deleteConcertDetails', ['concert_details' => $concert_details]) }}" class="font-medium  hover:underline">Delete</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
