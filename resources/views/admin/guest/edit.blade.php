...
<div class="relative flex justify-center overflow-x-auto  sm:rounded-lg">
    <div class="w-1/3 p-10 bg-[#1A1A1A] shadow-lg rounded-lg flex-col justify-center items-center">
        <header class="pb-10 flex justify-evenly items-center">
            <img class="h-40 object-contain w-20 rounded-lg shadow-xl" src="{{ asset('storage/' . $guest->image) }}" alt="">
            <div>
        ...
        <form method="POST" action="{{ route('admin.updateGuest',['guest' => $guest]) }}"  enctype="multipart/form-data">
 
             @csrf
             <div class="mb-6">
                 <label class="block mb-2 text-sm font-medium text-white dark:text-white" for="name">Name</label>
                 <input type="text" id="name" name="name" value="{{ $guest->name }}" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5" >
                 @error('name')
             ...
             <div class="mb-6">
                <label class="block mb-2 text-sm font-medium text-white dark:text-white" for="pquote">Personal Quote</label>
                <input type="text" id="pquote" value="{{ $guest->pquote }}" name="pquote" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5 " >
                @error('pquote')
                <p class="mt-1 text-sm text-red-600 dark:text-gray-200" id="file_input_help">{{ $message }}</p>
                @enderror
            </div>
