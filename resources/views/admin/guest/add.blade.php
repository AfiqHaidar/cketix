...
<div class="relative flex justify-center overflow-x-auto  sm:rounded-lg">
    <div class="w-1/3 p-10 bg-[#1A1A1A] shadow-lg rounded-lg flex-col justify-center items-center">
        ...
        <form method="POST" action="{{ route('admin.createGuest') }}"  enctype="multipart/form-data">
             ...
             <div class="mb-6">
                ...
                <input type="text" id="pquote" name="pquote" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5 " >
                @error('pquote')
                <p class="mt-1 text-sm text-red-600 dark:text-gray-200" id="file_input_help">{{ $message }}</p>
                @enderror
            </div>
