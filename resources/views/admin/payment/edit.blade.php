...
<div class="relative flex justify-center overflow-x-auto  sm:rounded-lg">
    <div class="w-1/3 p-10 bg-[#1A1A1A] shadow-lg rounded-lg flex-col justify-center items-center">
        ...
        <form method="POST" action="{{ route('admin.updatePayment',['payment' => $payment]) }}"  enctype="multipart/form-data">
 
            @csrf
            <div class="mb-6">
                <label class="block mb-2 text-sm font-medium text-white dark:text-white" for="payment">New Payment Method Name</label>
                <input type="text" id="payment" name="payment" value="{{ $payment->name }}" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5" >
