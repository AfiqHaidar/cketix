<x-app-layout>
    ...
    <div class="px-4 w-3/4">
        <div class="px-5 py-2 mx-auto my-3 hover:text-white bg-gray-300 hover:bg-white shadow-lg rounded-lg w-fit">
            <a href=" {{ route('admin.addPayment') }}"  class="text-black">Add Payment Method</a>
        </div>
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b hover:bg-gray-100 ">
                                <th scope="row" class="text-center px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    PM{{ str_pad($payment->id, 3, '0', STR_PAD_LEFT); }}
                                </th>
                                <td class="text-center px-6 py-4">
                                    {{ $payment->payment }}
                                </td>
                                <td class="flex justify-evenly items-center px-6 py-4 text-right">
                                    <a href="{{ route('admin.editPayment', ['id' => $payment->id])  }}" class="font-medium  hover:underline">Edit</a>
                                    <a href="{{ route('admin.deletePayment', ['payment' => $payment]) }}" class="font-medium  hover:underline">Delete</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
