...
<div class="relative flex justify-center overflow-x-auto  sm:rounded-lg">
    <div class="w-1/3 p-10 bg-[#1A1A1A] shadow-lg rounded-lg flex-col justify-center items-center">
        <header class="pb-10 flex justify-evenly items-center">
            <img class="h-40 object-contain w-20 rounded-lg shadow-xl" src="{{ asset('storage/' . $banner->image) }}" alt="">
            <div>
        ...
        <form method="POST" action="{{ route('admin.updateBanner',['id' => $banner->id]) }}"  enctype="multipart/form-data">
             ...
             <div class="mb-6">
                 ...
                 <input type="text" id="header" name="header" value="{{ $banner->header }}" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5" >
                 @error('banner')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="subheader" value="{{ $banner->subheader }}" name="subheader" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg  block w-full p-2.5 " >
                    @error('subheader')
                    <p class="mt-1 text-sm text-red-600 dark:text-gray-200" id="file_input_help">{{ $message }}</p>
                    @enderror
                </div>
